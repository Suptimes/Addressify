...
const ChatDetails = ({ userId, receiver, userBlockedList }: UserProp) => {
  ...
  const handleBlockOrUnblockUser = (userId, receiverId, setReceiverBlocked) => {
    ...
    blockOrUnblockUser(
      { userId, blockedUser: receiverId },
      {
        onSuccess: () => {
          setReceiverBlocked(prevState => !prevState)
          setIsBlocking(false)
          toast.success(!receiverBlocked ? "User blocked successfully" : "User unblocked successfully")
        },
        onError: (error) => {
          setIsBlocking(false)
